...
board::operator std::string() const{
   ...
   int d=9;
   cout<<"                                                  "<<endl;
   ss<<" Random Agent Current Score = "<<woc<<"         "<<"Distance Agent Current Score = "<<boc<<endl;
...
string board::traverseHorizontal() const{
    ...
    return traverse(start,0,5,1); //startCell , primary , secondary direction , traverse direction
} 
...
void board::refreshOffboardCounts(){
 ...
 void board::winner(int boc,int woc){
    if(boc<woc){
        cout<<"Winner  White Marble : "<<woc<<endl;
    }
    else{
        cout<<"Winner Black Marble: "<<boc<<endl;
    }
}
